...
In 1852, Francis Guthrie, a mathematics student at UCL, noticed that four colours seemed sufficient to colour any map so that no two adjacent regions shared the same colour. He shared this observation with his brother Frederick, who then brought it to the attention of their professor, Augustus De Morgan.
...
One way to see this is to first think about how we can draw $K_3$ in the plane. Clearly this must form a triangle. Now if we want to add a fourth vertex to create $K_4$, we can place it either inside or outside this triangle. However in both cases our drawing of $K_4$ will consist of a triangle with a single central vertex inside it, and  the edges split the  large triangle into three smaller triangles (so it looks something like Figure \ref{K4}).
...
V- E+ F \ge 0.
\end{equation}
We also need to use the fact that if a graph has at least 3 vertices and no loops or multiple edges then $3F \le 2E$. (This is essentially because every face of a graph contains at least 3 edges, and each edge belongs to exactly 2 faces.)

Putting these two facts together we can check whether $K_8$ can be drawn on a torus without edges crossing. Since $K_8$ has 8 vertices and $\binom{8}{2} = 28$ edges, if we could draw it on a torus without edges crossing then we would have $V=8$ and $E=28$. Using the inequality $3F \le 2E$ we have $F \le \frac{2E}{3} = \frac{2 \cdot 28}{3} = \frac{56}{3}$. Putting this into the Euler characteristic formula we have $V - E + F \le 8 - 28 + \frac{56}{3} = -\frac{4}{3} < 0$, which contradicts the Euler characteristic formula (\ref{EC:eq}). So $K_8$ cannot be drawn on a torus without edges crossing.
