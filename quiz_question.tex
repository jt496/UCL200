...
Putting these two facts together we can check whether $K_8$ can be drawn on a torus without edges crossing. Since $K_8$ has 8 vertices and 28 edges, if we could draw it on a torus without edges crossing then we would have $V=8$ and $E=28$. Using the inequality $3F \le 2E$ we have $F \le \frac{2E}{3} = \frac{2 \cdot 28}{3} = \frac{56}{3}$. Putting this into the Euler characteristic formula we have $V - E + F \le 8 - 28 + \frac{56}{3} = \frac{8}{3} < 0$, which is a contradiction. So $K_8$ cannot be drawn on a torus without edges crossing.
